...
<fmt:bundle basename="org.wso2.carbon.cassandra.cluster.mgt.ui.i18n.Resources">
    ...
    <carbon:breadcrumb
            label="cassandra.cluster.node.gossipInfo"
            resourceBundle="org.wso2.carbon.cassandra.cluster.mgt.ui.i18n.Resources"
            topPage="false"
            request="<%=request%>"/>
    <div id="middle">
    <h2><fmt:message key="cassandra.cluster.node.gossipInfo" />-<%if(!"unknown".equalsIgnoreCase(hostName) && hostName!=null){%>
        <%=hostName%>
        <%}else{%>
        <%=hostAddress%>
        <%}%></h2>
    ...
    <table class="styledLeft" id="gossipInfoTable">
        <thead>
            <tr>
                <th colspan="1"><fmt:message key="cassandra.cluster.node.gossipInfo" /></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <%for(String s:gossipInfo){%>
                    <%=s%>
                    <br/>
                    <%}%>
                </td>

...
<%
    ...
    try{
        ...
        sstables=clusterColumnFamilyStatsAdminClient.getSSTables(hostAddress,keyspace,columnFamily,key);
    }
    catch (Exception e) {
...
<fmt:bundle basename="org.wso2.carbon.cassandra.cluster.mgt.ui.i18n.Resources">
    ...
    <carbon:breadcrumb
            label="cassandra.cluster.node.ssTables"
            resourceBundle="org.wso2.carbon.cassandra.cluster.mgt.ui.i18n.Resources"
            topPage="false"
            request="<%=request%>"/>
    <div id="middle">
        <h2><fmt:message key="cassandra.cluster.node.ssTables" />(<fmt:message key="cassandra.cluster.column.family" />-<%=columnFamily%>)</h2>
        <div id="workArea">
            <table class="styledLeft" id="gossipInfoTable">
                <thead>
                <tr>
                    <th colspan="2"><fmt:message key="cassandra.cluster.node.ssTables" /></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <td>
                        ...
                        <%
                            if(sstables.length>0 && sstables[0]!=null){
                                for(String s:sstables){%>
                        <%=s%>
                        <br/>
                        <%}}else{%>
